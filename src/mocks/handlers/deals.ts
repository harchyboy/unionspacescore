...
import type { DealRoomResponse, DealRoomDeal, DealRoomSummary } from '../../types/dealRoomDashboard';
...
function filterDeals(deals: DealRoomDeal[], filters: URLSearchParams): DealRoomDeal[] {
  let filtered = [...deals];

  const stage = filters.get('stage');
  if (stage && stage !== 'all') {
    filtered = filtered.filter((d) => d.stage === stage);
  }

  const owner = filters.get('owner');
  if (owner && owner !== 'all') {
    filtered = filtered.filter((d) => d.owner.id === owner);
  }

  const dealType = filters.get('dealType');
  if (dealType && dealType !== 'all') {
    filtered = filtered.filter((d) => d.type === dealType);
  }

  const property = filters.get('property');
  ...
  if (search) {
    ...
    filtered = filtered.filter(
      (d) =>
        ...
        d.tenant.companyName.toLowerCase().includes(searchLower) ||
        (d.requirementCode && d.requirementCode.toLowerCase().includes(searchLower))
    );
  }
...
export const dealsHandlers = [
  http.get('/api/deals', ({ request }) => {
    ...
    const filteredDeals = filterDeals(mockDeals, url.searchParams);
